...
function addScheduleField(schedule = {}) {
    ...
    field.innerHTML = `
        <select name="schedule-day" class="w-full px-3 py-2 bg-bkg text-secondary border border-border rounded-md">
            ...
            <option value="Qui">Quinta</option><option value="Sex">Sexta</option><option value="Sab">Sábado</option><option value="Dom">Domingo</option>
        </select>
...
export { addScheduleField, addGradeField, updateWeightsSum, renderGradeFields };

function showModal(modalElement) { if (modalElement) modalElement.classList.remove('hidden', 'opacity-0'); }
function hideModal(modalElement) { if (modalElement) modalElement.classList.add('hidden', 'opacity-0'); }
  ...
  if (enrollmentId) {
    ...
    api.getEnrollment(enrollmentId).then(docSnap => {
        if(docSnap.exists()) {
            ...
            // CORRIGIDO: Preenche o campo da média de aprovação
...
export async function showPeriodOptionsModal() {
    ...
    if (dom.periodOptionsTitle) dom.periodOptionsTitle.textContent = `Opções do Período "${currentPeriod.name}"`;
...
export function showConfirmDeleteModal(item) {
  if (!dom.confirmDeleteModal || !dom.confirmDeleteMessage) return;
  setState('itemToDelete', item);
  let message = "Tem certeza que deseja excluir? Esta ação não pode ser desfeita.";
  if (item.type === 'enrollment') message = "Excluir esta matrícula também removerá todos os seus períodos e disciplinas.";
  else if (item.type === 'discipline') message = "Tem certeza que deseja excluir esta disciplina?";
  else if (item.type === 'period') message = "Excluir este período também removerá todas as suas disciplinas.";
